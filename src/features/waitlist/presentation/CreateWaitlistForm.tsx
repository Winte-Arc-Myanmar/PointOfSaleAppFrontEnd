...
import { z } from "zod";
import { useCreateWaitlistEntry } from "@/presentation/hooks/useWaitlist";
  ...
  const submit = (data: FormData) => {
    create.mutate(
      ...
      },
      {
        ...
        onError: () => toast.error("Failed to add waitlist entry."),
      },
    );
  };
    ...
    <form id={formId} onSubmit={form.handleSubmit(submit)} className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="grid gap-2">
          ...
          <Input id="guestName" {...form.register("guestName")} placeholder="John Smith" />
        </div>
        <div className="grid gap-2">
