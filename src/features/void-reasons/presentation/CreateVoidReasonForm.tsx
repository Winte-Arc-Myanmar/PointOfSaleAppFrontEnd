...
import { z } from "zod";
import { useCreateVoidReason } from "@/presentation/hooks/useVoidReasons";
...
export interface CreateVoidReasonFormProps {
  onSuccess?: () => void;
  formId?: string;
  onLoadingChange?: (loading: boolean) => void;
}

export function CreateVoidReasonForm({
  onSuccess,
  formId,
  onLoadingChange,
}: CreateVoidReasonFormProps) {
  const { tenantId: lockedTenantId } = usePermissions();
  const create = useCreateVoidReason();
  ...
  const onSubmit = (data: FormData) => {
    create.mutate(
      ...
      },
      {
        ...
        onError: () => toast.error("Failed to create void reason."),
      },
    );
  };
    ...
    <form id={formId} onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="grid gap-2">
          ...
          <Input id="code" {...form.register("code")} placeholder="CUSTOMER_CHANGED_MIND" />
      ...
      <div className="grid gap-2">
        ...
        <Input id="name" {...form.register("name")} placeholder="Customer changed their mind" />
      ...
      <div className="grid gap-2">
        ...
        <Input
          ...
          placeholder="Used when the customer modifies their order before service"
        />
      </div>
